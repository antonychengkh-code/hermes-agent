...
- Lightpanda: https://lightpanda.io (search, extract — local browser or cloud)
...
_LEGACY_WEB_BACKENDS = frozenset(
    {"parallel", "firecrawl", "exa", "searxng", "brave-free", "ddgs", "xai", "keenable", "lightpanda"}
)
...
def _lightpanda_available() -> bool:
    """Return True when Lightpanda can serve (cloud token OR local binary).

    Mirrors the provider's ``is_available()`` OR its structural free-tier
    prerequisite: a ``LIGHTPANDA_TOKEN`` routes extraction through
    Lightpanda Cloud, while an installed ``lightpanda`` binary serves both
    capabilities locally with no key at all.
    """
    if _has_env("LIGHTPANDA_TOKEN"):
        return True
    try:
...
def _ensure_web_plugins_loaded() -> None:
    ...
    firecrawl, lightpanda) registers itself via ``plugins/web/<vendor>/__init__.py``
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        # (brave-free, ddgs, searxng, exa, parallel, firecrawl, lightpanda)
...
async def web_extract_tool(
    ...
    extract backends (Firecrawl, Exa, Parallel, Lightpanda) already return clean,
    ...
    try:
        ...
        if not safe_urls:
            ...
            # firecrawl, lightpanda) now live as plugins. The dispatcher is a
...
if __name__ == "__main__":
    ...
    if web_available:
        ...
        if backend == "exa":
            ...
            if _has_env("LIGHTPANDA_TOKEN"):
                print("   Using Lightpanda Cloud (https://lightpanda.io)")
            else:
                print("   Using Lightpanda local browser (https://lightpanda.io)")
