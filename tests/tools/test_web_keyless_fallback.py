...
class TestResolutionOrder:
    ...
    def test_keyless_ring_rotates_and_covers_all_vendors(self, fresh_registry, monkeypatch):
        ...
        # Force the structural usability gate open so the rotation
        # assertion is machine-independent (lightpanda needs its binary).
        monkeypatch.setattr(keyless_mcp, "_vendor_usable", lambda name: True)
    ...
    def test_check_web_api_key_false_when_disabled(self, fresh_registry, monkeypatch):
        ...
        monkeypatch.setattr(web_tools, "_ensure_web_plugins_loaded", lambda: None)
        monkeypatch.setattr(web_tools, "check_firecrawl_api_key", lambda: False)
        # Hermetic: a dev-box lightpanda binary is a real zero-key backend
        # and would legitimately flip this True.
        monkeypatch.setattr(
            "plugins.web.lightpanda.provider.find_lightpanda_binary", lambda: None
        )
...
class TestKeylessFailover:
    ...
    def test_failover_respects_peer_paid_pin(self, monkeypatch):
        ...
        for vendor in ("parallel", "firecrawl", "keenable", "lightpanda"):
            monkeypatch.setitem(
                keyless_mcp._KEYLESS_SEARCHERS, vendor,
                lambda q, l, v=vendor: self._throttled(v),
            )
