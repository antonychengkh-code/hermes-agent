...
| **Lightpanda** | `LIGHTPANDA_TOKEN` (optional) | ✔ | ✔ | ✔ Free with the local `lightpanda` binary (on-device browser); token unlocks the hosted cloud fleet |
...
web:
  search_backend: "searxng"
  extract_backend: "firecrawl"   # or lightpanda, exa, parallel
```
...
An [AI-native headless browser](https://lightpanda.io) written in Zig — no Chromium, 9x faster and 16x lighter. Two modes:

- **Free (local browser):** install the binary once — `curl -fsSL https://pkg.lightpanda.io/install.sh | bash` — and `web_extract` renders pages **on your machine** with real JavaScript execution, while `web_search` uses the binary's built-in search (Keenable's keyless public endpoint upstream). No account, no key.
- **Cloud (API token):** set `LIGHTPANDA_TOKEN` to route extraction through Lightpanda's hosted browser fleet (proxies, regions, no local process).

```bash
# optional — the local binary needs no token at all
# ~/.hermes/.env
LIGHTPANDA_TOKEN=your-cloud-token
```

Get a token at [console.lightpanda.io](https://console.lightpanda.io/signup).

Empty installs keep Firecrawl as the named default; with the binary installed, Lightpanda also joins the keyless ring automatically.
...
**Search-only** — pair with Firecrawl / Lightpanda / Exa / Parallel if you also need `web_extract`. On 401 the provider performs a single forced OAuth-token refresh and retries (covers mid-window revocation and opaque tokens the proactive expiry check can't decode); env-var credentials skip the retry.
...
# ~/.hermes/config.yaml
web:
  backend: "searxng"   # firecrawl | searxng | brave-free | ddgs | lightpanda | exa | parallel | xai
```
...
|--------------------|-----------------------|
| `LIGHTPANDA_TOKEN` (or the local binary) | lightpanda |
| `EXA_API_KEY` | exa |
...
| *(nothing set at all)* | keyless ring: exa / parallel / firecrawl / keenable / lightpanda (round-robin) |

**Keyless free-tier ring:** when *no* credential above is present, requests rotate across the ring vendors' public free tiers (Exa, Parallel, Firecrawl, Keenable, and Lightpanda when its binary is installed) so web tools work on a fresh install with zero setup — and a rate-limited request fails over to the next vendor in the ring automatically. Pin one vendor in `hermes tools` to stop the rotation (the ring is then only used as failover succession on throttles). All free tiers are vendor-rate-limited under burst load; sustained normal usage goes through fine. Set `web.keyless_fallback: false` to turn the tier off — with it off and no credentials, web tools are unavailable until a provider is configured.
